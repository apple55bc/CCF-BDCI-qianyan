...
from bd_chat.data_deal.input_ct import CTInput
from bd_chat.data_deal.trans_output import TransOutput
from bd_chat.model.model_context import ModelContext
from bd_chat.model.model_recall import SearchEMb
import jieba
import time
import numpy as np
import re
    ...
    if answer[0] == '[':
        for j in range(1, 4):
            if answer[j] == ']':
                e_i = j + 1
                break
    answer_split = answer[:e_i] + ' ' + ' '.join(jieba.lcut(answer[e_i:]))
print('\n=====> Over: ', i)
