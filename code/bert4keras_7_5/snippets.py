...
class AutoRegressiveDecoder(object):
    ...
    def beam_search(self, inputs, topk, temperature=False, score_decay_rate=0):
        """beam search解码
        ...
        for step in range(self.maxlen):
            ...
            if score_decay_rate > 0:
                if score_decay is not None:
                    scores = scores + np.log(score_decay)
                else:
                    score_decay = np.ones_like(scores)
                score_decay_new = scores.max(axis=1)[:, None] * np.ones(scores.shape[1])
                decay_idx = (score_decay_new == scores) * 1
                score_decay_new = 1 - decay_idx * score_decay_rate  # 一次重复则增加衰减
                if last_decay_idx is None:
                    last_decay_idx = np.zeros_like(decay_idx)
                score_decay = (1 - decay_idx * last_decay_idx * (1 - score_decay)) * score_decay_new  #累积重复累积衰减
            ...
            # 记录decay部分
            if score_decay_rate > 0:
                last_decay_idx = indices_2.reshape(-1)
                last_decay_idx = (np.arange(scores.shape[1])[None, :] == last_decay_idx[:, None]) * 1
                score_decay = score_decay[indices_1]
            ...
            if output_ids.shape[1] >= self.minlen:  # 最短长度判断
                ...
                if indices_2[best_one, 0] == self.end_id:  # 如果已经终止
                    ...
                    if not flag.all():  # 如果有已完成的
                        ...
                        if score_decay_rate > 0:
                            score_decay = score_decay[flag]  # 扔掉已完成序列
                            last_decay_idx = last_decay_idx[flag]
                        topk = flag.sum()  # topk相应变化
        # 达到长度直接输出
        return output_ids[output_scores.argmax()]

    def nucleus_sample(self, inputs, n, p=0.90, temperature=False, topk=None, min_k=1):
        inputs = [np.array([i]) for i in inputs]
        ...
        for step in range(self.maxlen):
            ...
            indices = (-np.array(probas)).argsort(axis=1)
            probas = np.take_along_axis(np.array(probas), indices, axis=1)
...
class Hook:
    ...
    def __getattr__(self, attr):
        """使得 from bert4keras_7_5.backend import uniout
